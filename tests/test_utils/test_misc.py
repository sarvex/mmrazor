...
def test_find_latest_checkpoint():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with open(f'{tmpdir}/latest.pth', 'w') as f:
            f.write('latest')
        path = tmpdir
        latest = find_latest_checkpoint(path)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with open(f'{tmpdir}/iter_4000.pth', 'w') as f:
            f.write('iter_4000')
        with open(f'{tmpdir}/iter_8000.pth', 'w') as f:
            f.write('iter_8000')
        path = tmpdir
        latest = find_latest_checkpoint(path)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with open(f'{tmpdir}/epoch_1.pth', 'w') as f:
            f.write('epoch_1')
        with open(f'{tmpdir}/epoch_2.pth', 'w') as f:
            f.write('epoch_2')
        path = tmpdir
        latest = find_latest_checkpoint(path)
